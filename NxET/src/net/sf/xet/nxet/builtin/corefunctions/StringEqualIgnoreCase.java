 ...
 */
public class StringEqualIgnoreCase extends Builtin {
     ...
     */
    public StringEqualIgnoreCase(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public StringEqualIgnoreCase(Node builtinAtom, World world, Integer place) {
     ...
     * (using Java's StringEqualIgnoredCase). If two
     * strings cannot be compared, (the java function
     * returns an exception), it will also return FALSE 
    ...
    public ExecutionResult execute() throws NotExecutableException {
        ...
        try {
	        result = builtinAtom.attributeByLocalName(ATTR_STRING1).getNodeValue().equalsIgnoreCase(
	        	     builtinAtom.attributeByLocalName(ATTR_STRING2).getNodeValue());
			execResult.setSuccess(result);
			execResult.setResultDescription(" [" + result + " = \"" +
					builtinAtom.attributeByLocalName(ATTR_STRING1).getNodeValue() + "\" equals \"" +
					builtinAtom.attributeByLocalName(ATTR_STRING2).getNodeValue() + "\" (case ignored)]");
	    } catch (Exception e) {
            e.printStackTrace();
        }
        return execResult;
