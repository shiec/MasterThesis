...
public class Minimum extends Builtin {
     ...
     */
    public Minimum(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public Minimum(Node builtinAtom, World world, Integer place) {
    ...
    public ExecutionResult execute() throws NotExecutableException {
		...
		execResult.setResultDescription(" [" + resultVariable + " <- " + df.format(result) + " = min(" +
				builtinAtom.attributeByLocalName(ATTR_NUMBER1).getNodeValue() + ", " +
				builtinAtom.attributeByLocalName(ATTR_NUMBER2).getNodeValue() + ")]");
