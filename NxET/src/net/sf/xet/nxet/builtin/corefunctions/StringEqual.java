 ...
 */
public class StringEqual extends Builtin {
     ...
     */
    public StringEqual(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public StringEqual(Node builtinAtom, World world, Integer place) {
     ...
     * (using Java's StringEqual). If two strings cannot 
     * be compared, (the java function returns an exception), 
     * it will also return FALSE execution success result.
     * 
    ...
    public ExecutionResult execute() throws NotExecutableException {
        ...
        try {
	        result = builtinAtom.attributeByLocalName(ATTR_STRING1).getNodeValue().equals(
			...
			execResult.setResultDescription(" [" + result + " = \"" +
					...
					builtinAtom.attributeByLocalName(ATTR_STRING1).getNodeValue() + "\"]");
	    } catch (Exception e) {
