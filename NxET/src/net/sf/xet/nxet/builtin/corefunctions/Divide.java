...
/**
 * Divide function divides<br>
 * <br>
 * - parameter number1 with<br>
 ...
 * can be divided successfully. Otherwise, it
 * will return FALSE.<br>
 * <br>
 * The Divide built-in can be used as a D-atom
 * or an N-atom.
 *
...
public class Divide extends Builtin {
    ...
    public final String ATTR_OPT_ARGUMENT1 = "roundType";
     ...
     */
    public Divide(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public Divide(Node builtinAtom, World world, Integer place) {
    ...
    public ExecutionResult execute() throws NotExecutableException {
		...
		if (builtinAtom.hasAttributeByLocalName(ATTR_OPT_ARGUMENT1)) {
			if (builtinAtom.attributeByLocalName(ATTR_OPT_ARGUMENT1).getNodeValue() == "round") {
				evaluationResult = Math.round(evaluationResult) * 1.00;
			} else if (builtinAtom.attributeByLocalName(ATTR_OPT_ARGUMENT1).getNodeValue() == "floor") {
				evaluationResult = Math.floor(evaluationResult);
			} else if (builtinAtom.attributeByLocalName(ATTR_OPT_ARGUMENT1).getNodeValue() == "ceil") {
				evaluationResult = Math.ceil(evaluationResult);
			}
		}
