 ...
 */
public class FirstElement extends Builtin {

	public static final String ATTR_NUMBER = "number";
    public static final String ELEMENT_LIST = "List";
    public static final String ELEMENT_FIRST = "First";
     ...
     */
    public FirstElement(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public FirstElement(Node builtinAtom, World world, Integer place) {
    ...
    public Boolean isMatched() {
        return new Boolean(((builtinAtom.numberOfChildNodes() == 2) || (builtinAtom.numberOfChildNodes() == 3)) &&
                		    ...
                		    (builtinAtom.childNode(1).getNodeName().equals(ELEMENT_FIRST)) &&
     ...
     * The first n element(s) of the list is copied to the "first"
    ...
    public ExecutionResult execute() throws NotExecutableException {
		...
		Node firstElement = builtinAtom.childNode(1);
		Node restElements = null;
		
		String firstVariableName = builtinAtom.childNode(1).childNode(0).getNodeName();
		...
		if (number <= listOfElements.numberOfChildNodes()) {
			for (int i = 0; i < number; i++) {
				firstElement.addChildNode(listOfElements.childNode(i).cloneNode());
			}
			if ((restElements != null) && (number >= 0)) {
				for (int i = number; i < listOfElements.numberOfChildNodes(); i++) {
		...
		spec.addSpecialization(firstVariableName, firstElement);
