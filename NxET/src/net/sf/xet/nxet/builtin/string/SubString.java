 ...
 */
public class SubString extends Builtin {

    public final String ATTR_STRING = "string";
    public final String ATTR_START = "startAt";
    public final String ATTR_END = "endAt";
     ...
     */
    public SubString(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public SubString(Node builtinAtom, World world, Integer place) {
    ...
    public Boolean isExecutable() {
        ...
        return new Boolean(!builtinAtom.attributeByLocalName(ATTR_STRING).isVariable() &&
        				   ((builtinAtom.hasAttributeByLocalName(ATTR_START))? !builtinAtom.attributeByLocalName(ATTR_START).isVariable(): true) &&
						   ((builtinAtom.hasAttributeByLocalName(ATTR_END))? !builtinAtom.attributeByLocalName(ATTR_END).isVariable(): true));
    }
    ...
    public ExecutionResult execute() throws NotExecutableException {
		...
		int indexEnd = 0;
        try {
        	if (builtinAtom.hasAttributeByLocalName(ATTR_START)) {
				indexStart = Integer.parseInt(builtinAtom.attributeByLocalName(ATTR_START).getNodeValue());
			} else {
				indexStart = 0;
			}
			if (builtinAtom.hasAttributeByLocalName(ATTR_END)) {
				indexEnd = Integer.parseInt(builtinAtom.attributeByLocalName(ATTR_END).getNodeValue());
			} else {
				indexEnd = originalString.length() - 1;
			}
        } catch (NumberFormatException e) {
        	throw new NotExecutableException(NotExecutableException.UNKNOWN_REASON, "Invalid index parameter supplied");
        }
        String result = originalString.substring(indexStart, indexEnd);
