 ...
 */
public class InsertString extends Builtin {

    public final String ATTR_STRING = "string";
	public final String ATTR_INSERTSTRING = "insertString";
	public final String ATTR_INDEX = "index";
     ...
     */
    public InsertString(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public InsertString(Node builtinAtom, World world, Integer place) {
    ...
    public Boolean isMatched() {
        ...
        return new Boolean(builtinAtom.hasAttributeByLocalName(ATTR_STRING) &&
						   builtinAtom.hasAttributeByLocalName(ATTR_INSERTSTRING) &&
						   builtinAtom.hasAttributeByLocalName(ATTR_INDEX) &&
    ...
    public Boolean isExecutable() {
        return new Boolean(!builtinAtom.attributeByLocalName(ATTR_STRING).isVariable() &&
						   !builtinAtom.attributeByLocalName(ATTR_INSERTSTRING).isVariable() &&
						   !builtinAtom.attributeByLocalName(ATTR_INDEX).isVariable() &&
    ...
    public ExecutionResult execute() throws NotExecutableException {
		...
		String insertString = builtinAtom.attributeByLocalName(ATTR_INSERTSTRING).getNodeValue();
		int index = Integer.parseInt(builtinAtom.attributeByLocalName(ATTR_INDEX).getNodeValue());
		if (index > originalString.length()) {
			throw new NotExecutableException(NotExecutableException.INVALID_PARAMETERS, "Invalid Insert Index");
		}
		String resultVariable = builtinAtom.attributeByLocalName(ATTR_RESULT).getNodeValue();
		String result = originalString.substring(0, index) + insertString + 
        ...
        execResult.setResultDescription(" [" + resultVariable + " <- \"" + result + "\" = \"" +
										originalString + "\".insert(\"" + insertString + 
										"\") at " + index + "]");
	    return execResult;
    }
