 ...
 * 
 */
package net.sf.xet.nxet.builtin.custom.corefunctions;
 ...
 */
public class SetUnion extends Builtin {
     ...
     */
    public SetUnion(Node builtinAtom, World world, Integer place, PrintStream out) {
     ...
     * @param place The place the built-in is executed
     */
    public SetUnion(Node builtinAtom, World world, Integer place) {
    ...
    public ExecutionResult execute() throws NotExecutableException {
        ...
        // Add all element in a first set into result set
        for (int j = 0; j < firstSet.numberOfChildNodes(); j++) {
        	resultSet.addChildNode(firstSet.childNode(j).cloneNode());
        }
        for (int i = 0; i < secondSet.numberOfChildNodes(); i++) {
            ...
            if(!found) {
            	resultSet.addChildNode(secondSet.childNode(i).cloneNode());
            }
        }
