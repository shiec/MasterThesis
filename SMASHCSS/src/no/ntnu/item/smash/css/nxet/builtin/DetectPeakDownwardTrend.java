 ...
 */
public class DetectPeakDownwardTrend extends Builtin {
	 ...
	 */
	public DetectPeakDownwardTrend(Node builtinAtom, World world, Integer place, PrintStream out) {
	 ...
	 *            The place the built-in is executed
	 */
	public DetectPeakDownwardTrend(Node builtinAtom, World world, Integer place) {
	...
	public ExecutionResult execute() throws NotExecutableException {
        ...
        for(int i=0; i<priceString.length-1; i++) {
        	double price = Double.parseDouble(priceString[i]);
        	double nextPrice = Double.parseDouble(priceString[i+1]);
        	if(price - nextPrice > difference){
