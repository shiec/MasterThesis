/*
 * This class will not be here if "schedule" is also maintained at CSS and synched with the schedule on the SIM platform
 */
...
public class EMUtility {
	...
	public static double estimatedHeatTime(double otemp, double rtemp,
		...
		double estTime = (-Math
				.log(-(((rtemp + deltaT) * uTotal) - 20 - transferFromMass - 20
						- watt - (uTotal * otemp))
						/ (20 + transferFromMass + 20 + watt - (rtemp * uTotal) + (uTotal * otemp))) * airMass)
				/ uTotal;
		//System.out.println(rtemp + " " + deltaT + " " + estTime);
	...
	public static double estimatedTempDropTime(double otemp, double rtemp,
		...
		double estTime = (-Math
				...
				/ uTotal;
		// System.out.println(estTime<=0?0:Math.round(estTime*100.0)/100.0);
