...
import no.ntnu.item.smash.sim.core.EMUtility;
...
public class Case3P2H_Hcom {
	...
	public static void main(String[] args) {
		...
		EMUtility.thermalProp = thermalProp;
		...
		// Configure DSO and DLC
		DSO.setContractType(1); // 0=DLC, 1=ULC
		DSO.addControlRequest("212010", new ULCRequest(24*1 + 17, 24*1 + 17, 2000));
		DSO.addControlRequest("1312010", new ULCRequest(24*12 + 17, 24*12 + 17, 2000));
		...
		sim.setGeneratePowerReductionRequest(true);
		...
		int id = context.getPgc().registerPolicy("Y:/Workspaces/SMASH/SMASHCSS/policies/p3_ulc.xet.xml", "comfort");
		ExternalEventTrigger trigger = new ExternalEventTrigger(Trigger.TYPE_EXTERNAL_EVENT, Trigger.COND_EQUALTO, "DSO");
		int id2 = context.getPgc().registerPolicy("Y:/Workspaces/SMASH/SMASHCSS/policies/p1_priceCom.xet.xml", "comfort");
		TimeTrigger trigger2 = new TimeTrigger(Trigger.TYPE_TIME, Trigger.COND_EQUALTO, 0);
		
		context.getPgc().registerPolicyTrigger(id, trigger);
		context.getPgc().registerPolicyTrigger(id2, trigger2);
		context.getPgc().activatePolicy(id);
		context.getPgc().activatePolicy(id2);
		
		sim.setCSSSuspended(false);
		sim.startSimulation(simTime, 1, 1, 2010, "experiments/Case3P2H_Hcom");
	}
